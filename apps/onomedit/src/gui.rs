...
use crate::i18n::{ALL_LANGUAGES, Language};
...
pub fn run() -> eframe::Result<()> {
    let language = Language::from_code(&config::load().language);
    let options = eframe::NativeOptions {
        viewport: egui::ViewportBuilder::default().with_inner_size([900.0, 640.0]),
        ..eframe::NativeOptions::default()
    };
    eframe::run_native(
        language.text(WINDOW_TITLE),
        options,
...
impl SettingsDraft {
    ...
    fn finish(mut self) -> Result<Config, String> {
        let language = Language::from_code(&self.config.language);
        self.config.editor_timeout = self
            .editor_timeout
            .trim()
            .parse()
            .map_err(|_| language.text("等待超时必须是数字").to_owned())?;
        self.config.subdirs_depth = self
            .subdirs_depth
            .trim()
            .parse()
            .map_err(|_| language.text("展开层级必须是整数").to_owned())?;
        Ok(self.config)
    }
}
...
impl OnomeditApp {
    fn new(creation: &eframe::CreationContext<'_>) -> Self {
        ...
        Self {
            ...
            raw_paths: Vec::new(),
            shown_paths: Vec::new(),
            status: language.text("就绪（可拖入文件或文件夹）").to_owned(),
            busy: false,
            view: View::Main,
            sender,
            receiver,
    ...
    fn start_plan(&mut self, context: &egui::Context, no_editor: bool, dry_run: bool) {
        ...
        if self.shown_paths.is_empty() {
            self.status = self.language().text("请先添加文件").to_owned();
            return;
        }
        ...
        self.status = if settings.open_editor {
            language.text("后台线程：准备文件并等待编辑器…").to_owned()
        } else {
            language.text("后台线程：正在生成重命名计划…").to_owned()
        };
            ...
            let result = (|| -> Result<(), String> {
                ...
                if settings.open_editor {
                    if settings.editor.trim().is_empty() {
                        return Err(language
                            .text("未配置编辑器，请先在设置中填写编辑器命令")
                            .to_owned());
                    }
                    send_event(
                        &sender,
                        &repaint,
                        WorkerEvent::Status(match language {
                            Language::ZhCn => format!(
                                "已写入临时文件: {}，等待编辑器保存并退出…",
                                session.edit_path().display()
                            ),
                            Language::EnUs => format!(
                                "Edit file created: {}; waiting for the editor to save and close…",
                                session.edit_path().display()
                            ),
                        }),
                    );
    ...
    fn execute_confirmation(&mut self, context: &egui::Context) {
        ...
        if confirmation.dry_run {
            self.status = self.language().text("预览模式不会执行重命名").to_owned();
            return;
        }
        let pairs = confirmation.selected_pairs();
        if pairs.is_empty() {
            self.status = self.language().text("未勾选任何项目").to_owned();
            return;
        }
        self.busy = true;
        self.status = self.language().text("后台线程：正在执行重命名…").to_owned();
        let sender = self.sender.clone();
        let repaint = context.clone();
        std::thread::spawn(move || {
            let logger = RenameLogger::new(config::log_dir());
        ...
        while let Ok(event) = self.receiver.try_recv() {
            match event {
                ...
                WorkerEvent::Planned {
                    ...
                    self.busy = false;
                    self.status = match (self.language(), dry_run) {
                        (Language::ZhCn, true) => format!("预览共 {} 项（未执行）", pairs.len()),
                        (Language::ZhCn, false) => {
                            format!("重命名计划共 {} 项，请确认", pairs.len())
                        }
                        (Language::EnUs, true) => {
                            format!("Preview: {} items (not applied)", pairs.len())
                        }
                        (Language::EnUs, false) => {
                            format!("Rename plan: {} items; review before applying", pairs.len())
                        }
                    };
                    self.view =
                ...
                WorkerEvent::Completed { result, operation } => {
                    self.busy = false;
                    self.status = format_result(self.language(), operation, &result);
                    self.view = View::Main;
                    self.refresh_paths();
    ...
    fn show_main(&mut self, context: &egui::Context, ui: &mut egui::Ui) {
        let language = self.language();
        ui.heading(language.text(WINDOW_TITLE));
        ui.add_space(6.0);
        ui.horizontal_wrapped(|ui| {
            if ui.button(language.text("添加文件…")).clicked()
                && let Some(paths) = rfd::FileDialog::new().pick_files()
            {
                self.add_paths(paths);
            }
            if ui.button(language.text("添加文件夹…")).clicked()
                && let Some(path) = rfd::FileDialog::new().pick_folder()
            {
                self.add_paths([path]);
            }
            if ui.button(language.text("从剪贴板")).clicked() {
                let paths: Vec<PathBuf> =
                    clipboard::get_paths().into_iter().map(Into::into).collect();
                if paths.is_empty() {
                    self.status = language.text("剪贴板为空或不可读").to_owned();
                } else {
                    self.add_paths(paths);
                }
            }
            if ui.button(language.text("清空")).clicked() {
                self.raw_paths.clear();
                self.shown_paths.clear();
                self.status = language.text("已清空").to_owned();
            }
            ui.separator();
            if ui.button(language.text("恢复上次")).clicked() {
                self.restore_last(context);
            }
            if ui.button(language.text("设置…")).clicked() {
        ...
        ui.horizontal_wrapped(|ui| {
            ...
            if ui
                .checkbox(&mut self.expand_subdirs, language.text("展开子文件夹"))
                .changed()
            {
                self.refresh_paths();
            }
            ui.label(language.text("层级:"));
            if ui
        ...
        ui.add_space(6.0);
        ui.horizontal_wrapped(|ui| {
            if ui.button(language.text("开始（打开编辑器）")).clicked() {
                self.start_plan(context, false, false);
            }
            if ui
                .button(language.text("直接应用规则（跳过编辑器）"))
                .clicked()
            {
                self.start_plan(context, true, false);
            }
            if ui.button(language.text("预览（进入重命名确认）")).clicked() {
        ...
        ui.group(|ui| {
            ...
            ui.label(language.text("文件（将按此顺序写入临时文件）"));
            ui.separator();
            egui::ScrollArea::vertical()
                ...
                .show(ui, |ui| {
                    if self.shown_paths.is_empty() {
                        ui.weak(language.text("拖入文件/文件夹，或使用上方按钮添加"));
                    }
    ...
    fn show_confirmation(&mut self, context: &egui::Context, ui: &mut egui::Ui) {
        ...
        ui.horizontal(|ui| {
            ...
            if confirmation.dry_run {
                ui.label(
                    RichText::new(language.text("只读，不会修改文件")).color(Color32::DARK_GREEN),
                );
            }
        });
        ui.separator();
        ...
        ui.horizontal(|ui| {
            if !confirmation.dry_run {
                if ui.button(language.text("全选")).clicked() {
                    confirmation
                        .rows
                        .iter_mut()
                        .for_each(|row| row.checked = true);
                }
                if ui.button(language.text("全不选")).clicked() {
                    confirmation
                        .rows
                        .iter_mut()
            ...
            let total = confirmation.rows.len();
            let selected = confirmation.rows.iter().filter(|row| row.checked).count();
            ui.label(match language {
                Language::ZhCn => format!("共 {total} 项，已选 {selected} 项"),
                Language::EnUs => format!("{total} items; {selected} selected"),
            });
            if ui
            ...
            if !confirmation.dry_run && ui.button(language.text("执行重命名")).clicked() {
                execute = true;
            }
        });
        ...
        egui::ScrollArea::both()
            ...
            .show(ui, |ui| {
                egui::Grid::new("confirmation_grid")
                    ...
                    .min_col_width(80.0)
                    .show(ui, |ui| {
                        ui.strong(language.text("执行"));
                        ui.strong(language.text("原文件名"));
                        ui.strong(language.text("新文件名"));
                        if confirmation.config.preview.diff {
                            ui.strong(language.text("差异"));
                        }
                        if confirmation.config.preview.distance {
                            ui.strong(language.text("距离"));
                        }
                        ui.end_row();
        ...
        if cancel {
            ...
            self.status = if dry_run {
                language.text("预览已关闭（未执行）").to_owned()
            } else {
                language.text("重命名确认已取消").to_owned()
            };
        } else if execute {
    ...
    fn show_settings(&mut self, ui: &mut egui::Ui) {
        ...
        ui.heading(language.text("Onomedit 设置"));
        ui.separator();
        let mut save_requested = false;
        let mut cancel_requested = false;
        let mut reset_requested = false;
        ui.horizontal(|ui| {
            if ui.button(language.text("保存")).clicked() {
                save_requested = true;
            }
            if ui.button(language.text("重置默认")).clicked() {
        ...
        egui::ScrollArea::vertical()
            ...
            .show(ui, |ui| {
                egui::Grid::new("settings_grid")
                    ...
                    .spacing([18.0, 5.0])
                    .show(ui, |ui| {
                        ui.label(language.text("语言"));
                        let selected_language = Language::from_code(&draft.config.language);
                        egui::ComboBox::from_id_salt("settings_language")
                            ...
                            .show_ui(ui, |ui| {
                                for value in ALL_LANGUAGES {
                                    ui.selectable_value(
                                        &mut draft.config.language,
                                        value.code().to_owned(),
                                        format!("{} ({})", value.name(), value.code()),
                                    );
                                }
                            });
                        ui.end_row();

                        setting_text(ui, language.text("主编辑器命令"), &mut draft.config.editor);
                        setting_text(
                            ui,
                            language.text("备用编辑器命令"),
                            &mut draft.config.editor_alt,
                        );
                        setting_text(
                            ui,
                            language.text("等待超时（秒）"),
                            &mut draft.editor_timeout,
                        );
                        setting_check(
                            ui,
                            language.text("多标签编辑器"),
                            &mut draft.config.multi_tab,
                        );
                        setting_check(
                            ui,
                            language.text("打开编辑器"),
                            &mut draft.config.open_editor,
                        );
                        setting_check(ui, language.text("应用规则"), &mut draft.config.apply_rules);
                        setting_check(
                            ui,
                            language.text("环境变量替换"),
                            &mut draft.config.enable_envvars,
                        );
                        setting_check(
                            ui,
                            language.text("自动替换规则"),
                            &mut draft.config.enable_auto_rules,
                        );
                        ...
                        setting_check(
                            ui,
                            language.text("反转顺序"),
                            &mut draft.config.sort_reverse,
                        );
                        setting_check(
                            ui,
                            language.text("展开子文件夹"),
                            &mut draft.config.expand_subdirs,
                        );
                        setting_text(ui, language.text("展开层级"), &mut draft.subdirs_depth);
                        setting_check(
                            ui,
                            language.text("跳过重命名确认"),
                            &mut draft.config.skip_confirmation,
                        );
                        setting_check(
                            ui,
                            language.text("完成后退出"),
                            &mut draft.config.exit_after,
                        );

                        setting_check(
                            ui,
                            language.text("排除文件"),
                            &mut draft.config.exclude.files,
                        );
                        setting_check(
                            ui,
                            language.text("排除目录"),
                            &mut draft.config.exclude.dirs,
                        );
                        setting_check(
                            ui,
                            language.text("排除符号链接"),
                            &mut draft.config.exclude.symlinks,
                        );
                        setting_check(
                            ui,
                            language.text("排除只读"),
                            &mut draft.config.exclude.readonly,
                        );
                        setting_check(
                            ui,
                            language.text("排除隐藏"),
                            &mut draft.config.exclude.hidden,
                        );
                        setting_check(
                            ui,
                            language.text("排除系统"),
                            &mut draft.config.exclude.system,
                        );

                        setting_check(
                            ui,
                            language.text("显示差异"),
                            &mut draft.config.preview.diff,
                        );
                        setting_check(
                            ui,
                            language.text("显示距离"),
                            &mut draft.config.preview.distance,
                        );
                        setting_check(
                            ui,
                            language.text("安全命名"),
                            &mut draft.config.safety.sanitize,
                        );
                    });
            });
        if reset_requested {
            ...
            self.view = View::Main;
            self.status = language.text("已取消设置（重置默认未保存）").to_owned();
        } else if cancel_requested {
            self.view = View::Main;
            self.status = language.text("设置未更改").to_owned();
            ...
            match draft.finish() {
                Ok(config) => match config::save(&config) {
                    Ok(()) => {
                        ...
                        self.refresh_paths();
                        self.view = View::Main;
                        self.status = self.language().text("设置已保存").to_owned();
                    }
                    Err(error) => {
                        self.status = match language {
                            Language::ZhCn => format!("保存设置失败: {error}"),
                            Language::EnUs => format!("Could not save settings: {error}"),
                        }
                    }
                },
                Err(error) => {
                    self.status = match language {
                        Language::ZhCn => format!("设置无效: {error}"),
                        Language::EnUs => format!("Invalid settings: {error}"),
                    }
                }
            }
        }
...
impl eframe::App for OnomeditApp {
    fn update(&mut self, context: &egui::Context, _frame: &mut eframe::Frame) {
            ...
            .show(context, |ui| {
                ui.horizontal(|ui| {
                    ...
                    let color = if self.status.starts_with("出错")
                        ...
                        || self.status.starts_with("Could not save settings")
                        || self.status.starts_with("设置无效")
                        || self.status.starts_with("Invalid settings")
                    {
                        Color32::RED
                    } else {
        ...
        if let Some(message) = self.alert.clone() {
            ...
            let language = self.language();
            egui::Window::new(language.text("目标重名，已中止"))
                .collapsible(false)
                .resizable(true)
                .open(&mut open)
                .show(context, |ui| {
                    ui.label(message);
                    ui.add_space(8.0);
                    if ui.button(language.text("关闭")).clicked() {
                        dismiss = true;
                    }
                });
            if dismiss || !open {
...
fn format_result(language: Language, operation: Operation, result: &RenameResult) -> String {
    let label = match operation {
        Operation::Rename => language.text("重命名完成"),
        Operation::Restore => language.text("恢复完成"),
    };
    match language {
        Language::ZhCn => format!(
            "{label}: 成功 {} / 失败 {} / 无变化 {}",
        ...
        Language::EnUs => format!(
            "{label}: {} succeeded / {} failed / {} unchanged",
            result.success.len(),
            result.failed.len(),
            result.skipped.len()
        ),
...
mod tests {
    ...
    fn result_status_reports_all_categories() {
        ...
        assert_eq!(
            format_result(Language::ZhCn, Operation::Rename, &result),
