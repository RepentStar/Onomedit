...
class MainWindow:
    ...
    def _build(self) -> None:
        ...
        # 文件列表
        list_frame = ttk.LabelFrame(main, text="文件（将按此顺序写入临时文件）", padding=6)
        ...
        ttk.Button(btns, text="添加文件…", command=self._pick_files).pack(side="left", padx=2)
        ttk.Button(btns, text="添加文件夹…", command=self._pick_dir).pack(side="left", padx=2)
        ttk.Button(btns, text="从剪贴板", command=self._from_clipboard).pack(side="left", padx=2)
        ttk.Button(btns, text="清空", command=self._clear).pack(side="left", padx=2)
        ttk.Button(btns, text="设置…", command=self._open_settings).pack(side="right", padx=2)
        ttk.Button(btns, text="恢复上次", command=self._restore_last).pack(side="right", padx=2)
        ...
        self.depth_spin = ttk.Spinbox(opts, from_=1, to=99, width=4, textvariable=self.depth_var, command=self._refresh)
        self.depth_spin.pack(side="left")

        # 执行
        run_frame = ttk.Frame(main)
        run_frame.pack(fill="x", pady=(4, 0))
        ttk.Button(run_frame, text="开始（打开编辑器）", command=self._start).pack(side="left")
        ttk.Button(run_frame, text="直接应用规则（跳过编辑器）", command=lambda: self._start(no_editor=True)).pack(
            side="left", padx=6
        )
        ttk.Button(run_frame, text="预览（不执行）", command=lambda: self._start(dry_run=True)).pack(side="left")
