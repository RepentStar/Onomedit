"""重命名主流程编排：收集→编辑器→读回→应用→重命名→日志。

流程（文档第三节）：

1. 解析参数 → 收集文件列表（参数/剪贴板，过滤不存在的路径）
2. 创建路径对象数组 → 展开子文件夹（可选）→ 应用排除规则过滤
3. 序列化写入临时文件（UTF-8）→ 启动外部编辑器等待编辑完成
4. 读回临时文件，校验行数与文件数一致（不一致必须中止）
5. 逐行反序列化 → 逐项应用规则（顺序固定）→ 逐项执行重命名
6. 保存日志，输出统计
"""
...
class Renamer:
    ...
    - 目标重复预检：同一目标被多个源映射时，后续项标记失败（不执行）。
    - 真冲突（目标已存在且不属于本批次）：安全序号化 ``名 (1).扩展名``。
    - 链/环（目标属于本批次尚未执行的源）：两阶段解环——主循环先把冲突目标
      移出到临时名并直接完成无冲突改名，被移出的内容在第二阶段落位到最终目标；
      临时名操作也会记入日志，确保恢复完整。
    - 单文件失败不中断批次：逐项捕获 OSError 记录失败。
    """
