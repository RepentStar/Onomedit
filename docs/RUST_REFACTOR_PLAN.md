...
- 当前 CI 仅运行 Windows 的 Python oracle、Rust format/check/test、完整和 CLI-only 编译；macOS/Linux CI 暂缓，待 Windows 版本稳定后恢复。
- 建立统一错误类型和应用层退出码映射。
- 引入依赖审计与许可证检查，生成并提交 `Cargo.lock`。

退出门槛：空骨架在 Windows 编译；CLI-only 构建不解析或链接 GUI 依赖；Windows CI 可缓存但不依赖本地工具状态。macOS/Linux 编译不作为当前阶段退出条件。
...
| --- | --- | --- | --- | --- | --- | --- |
| PathItem / safe name | 必需 | 必需 | - | 必需 | 暂缓 | 暂缓 |
| 规则 / 正则 / 占位符 | 必需 | 必需 | 选做 | 必需 | 暂缓 | 暂缓 |
| 收集 / glob / 展开 | 必需 | 必需 | 必需 | 必需 | 暂缓 | 暂缓 |
| 排除 / 文件属性 | 必需 | 必需 | 必需 | 必需 | 暂缓 | 暂缓 |
| 排序 / 去重 | 必需 | 必需 | 必需 | 必需 | 暂缓 | 暂缓 |
| 临时文件 / 编辑器 | 必需 | 必需 | 必需 | 必需 | 暂缓 | 暂缓 |
| 冲突 / 链 / 环 | 必需 | 必需 | 必需 | 必需 | 暂缓 | 暂缓 |
| 日志 / restore | 必需 | 必需 | 必需 | 必需 | 暂缓 | 暂缓 |
| CLI / completion | 必需 | 必需 | 必需 | 必需 | 暂缓 | 暂缓 |
| GUI / 拖拽 | 状态测试 | - | smoke | 必需 | 暂缓 | 暂缓 |
